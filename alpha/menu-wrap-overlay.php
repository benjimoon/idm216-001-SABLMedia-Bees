...
$query = "SELECT * FROM wraps WHERE id = {$_GET['id']}";
$result = mysqli_query($db_connection, $query);
if ($result->num_rows > 0) {
    // Get row from results and assign to $user variable;
    $wrap = mysqli_fetch_assoc($result);
} else {
...
?>
 ...
 $site_url = site_url();
          echo "
<img class='detail__item--image' src =' {$site_url}{$wrap['image_path2']}'>
";
?>
      </div>
      <div class="card-inner">
        <h2 class="detail__item--name"><?php echo $wrap['title']?></h2>
        <h2 class="detail__item--price"><?php echo $wrap['price']?> </h2>
        <p class="detail__item--description">
        <?php echo $wrap['description']?> <br /><br/>
        </p>
