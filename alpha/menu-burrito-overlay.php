...
$query = "SELECT * FROM burritos WHERE id = {$_GET['id']}";
$result = mysqli_query($db_connection, $query);
if ($result->num_rows > 0) {
    // Get row from results and assign to $user variable;
    $burrito = mysqli_fetch_assoc($result);
} else {
...
?>
 ...
 $site_url = site_url();
          echo "
<img class='detail__item--image' src =' {$site_url}{$burrito['image_path2']}'>
";
?>
      </div>
      <div class="card-inner">
        <h2 class="detail__item--name"><?php echo $burrito['title']?></h2>
        <h2 class="detail__item--price"><?php echo $burrito['price']?> </h2>
        <p class="detail__item--description">
        <?php echo $burrito['description']?> <br /><br/>
        </p>
