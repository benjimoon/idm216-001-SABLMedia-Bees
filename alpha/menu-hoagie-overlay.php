...
$query = "SELECT * FROM hoagies WHERE id = {$_GET['id']}";
$result = mysqli_query($db_connection, $query);
if ($result->num_rows > 0) {
    // Get row from results and assign to $user variable;
    $hoagie = mysqli_fetch_assoc($result);
} else {
...
?>
 ...
 $site_url = site_url();
          echo "
<img class='detail__item--image' src =' {$site_url}{$hoagie['image_path2']}'>
";
?>
      </div>
      <div class="card-inner">
        <h2 class="detail__item--name"><?php echo $hoagie['title']?></h2>
        <h2 class="detail__item--price"><?php echo $hoagie['price']?> </h2>
        <p class="detail__item--description">
        <?php echo $hoagie['description']?> <br /><br/>
        </p>
